...
impl Book {
    ...
    /// Returns the string corresponding to a number, title, and the numbering template for chapter
    #[doc(hidden)]
    pub fn get_chapter_header<F>(&self, n: i32, title: String, mut f: F) -> Result<HeaderData>
        where F: FnMut(&str) -> Result<String>
    {
        ...
        // let header_name = self
        //     .options
        //     .get_str(&format!("rendering.{}", "chapter"))
        //     .map(|s| s.to_owned())
        //     .unwrap_or_else(|_| lang::get_str(self.options.get_str("lang").unwrap(),
        //                                       "chapter"));

        // data = data.insert_str("chapter_title", title)
        //     .insert_str("chapter", header_name.clone())
        //     .insert_str("number", number.clone());
        // let data = data.build();
        // let mut res: Vec<u8> = vec![];
    ...
    #[doc(hidden)]
    pub fn get_part_header<F>(&self, n: i32, title: String, mut f: F) -> Result<HeaderData>
        where F: FnMut(&str) -> Result<String>
    {
        ...
        // let mut data = self.get_metadata(&mut f)?;
        // if !title.is_empty() {
        //     data = data.insert_bool("has_part_title", true);
        // }
        // let number = self.get_header_number(Header::Part, n)?;
        // data = data.insert_str("part_title", title)
        //     .insert_str("number", number);

        // let data = data.build();
        // let mut res: Vec<u8> = vec![];

        // if let Some(ref template) = self.part_template {
        //     template.render_data(&mut res, &data)?;
        // } else {
        //     let template =
